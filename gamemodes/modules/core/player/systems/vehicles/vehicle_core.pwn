...
LoadVehicles() {
    ...
    printf("[VEÕCULOS]: %d veÌculos carregados com sucesso.", loadedVehicles);
    return true;
}
...
ParkPlayerVehicle(playerid) {
    ...
    if(GetPlayerState(playerid) != PLAYER_STATE_DRIVER) return SendErrorMessage(playerid, "VocÍ deve ser o motorista do veÌculo para usar esse comando.");

    if(!VehicleIsOwner(playerid, id)) return SendErrorMessage(playerid, "VocÍ deve ser o dono do veÌculo para usar esse comando.");

    if(IsVehicleImpounded(vehicleid)) return SendErrorMessage(playerid, "Este veÌculo est· apreendido, portanto vocÍ n„o pode utiliz·-lo.");
    ...
    if(IsPlayerInRangeOfPoint(playerid, 5.0, vInfo[id][vPos][0], vInfo[id][vPos][1], vInfo[id][vPos][2]) && vInfo[id][vVW] == GetPlayerVirtualWorld(playerid) && vInfo[id][vInterior] == GetPlayerInterior(playerid)) {
        ...
        if(vInfo[id][vNamePersonalized]) SendServerMessage(playerid, "Seu veÌculo %s (( %s )) foi estacionado na vaga.", vInfo[id][vName], ReturnVehicleModelName(vInfo[id][vModel]));
		else SendServerMessage(playerid, "Seu veÌculo %s foi estacionado na vaga.", ReturnVehicleModelName(vInfo[id][vModel]));
        ...
        SendErrorMessage(playerid, "VocÍ n„o est· perto da sua vaga.");
        if(vInfo[id][vVW] == 0) {
            va_SendClientMessage(playerid, 0xFF00FFFF, "INFO: VocÍ pode usar a marca vermelha no mapa para achar a vaga do seu veÌculo.");
...
ChangeParkPlayerVehicle(playerid) {
    ...
    if(GetPlayerState(playerid) != PLAYER_STATE_DRIVER) return SendErrorMessage(playerid, "VocÍ deve ser o motorista do veÌculo para usar esse comando.");
    if(!VehicleIsOwner(playerid, id)) return SendErrorMessage(playerid, "VocÍ ser o dono do veÌculo para usar esse comando.");
    if(IsVehicleImpounded(id)) return SendErrorMessage(playerid, "Este veÌculo est· apreendido, portanto vocÍ n„o pode utiliz·-lo.");
    ...
    if(vInfo[id][vNamePersonalized]) SendServerMessage(playerid, "VocÍ atualizou a vaga do seu veÌculo %s (( %s )).", vInfo[id][vName], ReturnVehicleModelName(vInfo[id][vModel]));
	else SendServerMessage(playerid, "VocÍ atualizou a vaga do seu veÌculo %s.", ReturnVehicleModelName(vInfo[id][vModel]));
...
CheckVehicleStats(playerid) {
	...
	} else format(sunpassstatus, 128, "N„o instalado");
	...
	if(vInfo[id][vModel] == 481 || vInfo[id][vModel] == 509 || vInfo[id][vModel] == 510) {
		...
		va_SendClientMessage(playerid, COLOR_WHITECYAN, "Vida ˙til: Engrenagem[%.2f], Milhas Rodadas[%.1f]", vInfo[id][vEngine], vInfo[id][vMiles]);
	} else {
		if(vInfo[id][vNamePersonalized]) va_SendClientMessage(playerid, COLOR_WHITECYAN, "%s ((%s)):", vInfo[id][vName], ReturnVehicleModelName(vInfo[id][vModel]));
		else va_SendClientMessage(playerid, COLOR_WHITECYAN, "%s:", ReturnVehicleModelName(vInfo[id][vModel]));

		va_SendClientMessage(playerid, COLOR_WHITECYAN, "Trava[%d], Alarme[%d], Seguro[%d], Placa[%s], Sun Pass[%s]", vInfo[id][vLocked], vInfo[id][vAlarm], vInfo[id][vInsurance], vInfo[id][vPlate], sunpassstatus);
		va_SendClientMessage(playerid, COLOR_WHITECYAN, "Vida ˙til: Motor[%.2f], Bateria[%.2f], Milhas Rodadas[%.1f]", vInfo[id][vEngine], vInfo[id][vBattery], vInfo[id][vMiles]);
	}
    return true;
}
...
SetVehicleLock(playerid) {
	...
	if ((id = VehicleInside(playerid)) != -1 || (id = VehicleNearestToLock(playerid)) != -1) {
	    ...
	    if (VehicleIsOwner(playerid, id)) {
            ...
            if(vInfo[id][vNamePersonalized])  SendErrorMessage(playerid, "VocÍ n„o possui as chaves para destrancar o veÌculo %s (( %s )).",vInfo[id][vName], ReturnVehicleModelName(vInfo[id][vModel]));
            else SendErrorMessage(playerid, "VocÍ n„o possui as chaves para destrancar o veÌculo %s.", ReturnVehicleModelName(vInfo[id][vModel]));
        }
	} else SendErrorMessage(playerid, "VocÍ n„o est· prÛximo do seu veiculo.");
	return true;
}
...
ShowPlayerVehicles(playerid) {
    ...
    if(!cache_num_rows()) return SendErrorMessage(playerid, "VocÍ n„o possui nenhum veÌculo.");
	...
	new title[128];
	format(title, 128, "VeÌculos_de_%s", pNome(playerid));
		...
		for(new i; i < cache_num_rows(); i++) {
			...
			if(count == listitem) {
				if(vInfo[veh_id][vVehicle]) {
					SendErrorMessage(playerid, "Este veÌculo j· est· spawnado.");
					break;
				}
                if(veh_impounded) {
                    SendErrorMessage(playerid, "Este veÌculo est· apreendido e n„o pode ser spawnado.");
                    break;
                }

				LoadVehicle(veh_id);
				if(veh_pname != 0) SendServerMessage(playerid, "Seu veÌculo %s foi spawnado.", veh_name);
				else SendServerMessage(playerid, "Seu veÌculo %s foi spawnado.", ReturnVehicleModelName(veh_model));

				if(vInfo[i][vVW] == 0) {
					va_SendClientMessage(playerid, 0xFF00FFFF, "INFO: VocÍ pode usar a marca vermelha no mapa para achar seu veiculo.");
...
hook OnPlayerStateChange(playerid, newstate, oldstate) {
    if(oldstate == PLAYER_STATE_ONFOOT && newstate == PLAYER_STATE_DRIVER) {
        ...
        if (VehicleIsOwner(playerid, id)) {
            if(vInfo[id][vNamePersonalized]) va_SendClientMessage(playerid, COLOR_WHITECYAN, "Bem-vindo(a) ao seu veÌculo %s.", vInfo[id][vName]);
			else va_SendClientMessage(playerid, COLOR_WHITECYAN, "Bem-vindo(a) ao seu veÌculo %s.", ReturnVehicleModelName(vInfo[id][vModel]));
        }
        //SaveVehicle(id);
    }
    return true;
...
SellVehicleToDealer(playerid)
{
    if(IsPlayerInRangeOfPoint(playerid, 5.0, 542.0506, -1292.9080, 17.2422) || IsPlayerInRangeOfPoint(playerid, 5.0, 2131.8108, -1150.8969, 24.1069))
    {
        ...
        if(!IsPlayerInAnyVehicle(playerid)) return SendErrorMessage(playerid, "VocÍ n„o est· em um veÌculo!");
        if(vInfo[vehicle_id][vOwner] != pInfo[playerid][pID]) return SendErrorMessage(playerid, "VocÍ n„o È proprietario deste veÌculo!");

        vehicle_price = ((GetVehiclePrice(GetVehicleModel(GetPlayerVehicleID(playerid))) * 60) / 100);

        Dialog_Show(playerid, DialogSellVehicle, DIALOG_STYLE_MSGBOX, "Vender veÌculo", "VocÍ deseja vender seu veÌculo por $%s?", "Vender", "Cancelar", FormatNumber(vehicle_price));
    }
    else
    {
        SendErrorMessage(playerid, "VocÍ n„o est· em uma concessionaria!");
    }
    return 1;
}
...
Dialog:DialogSellVehicle(playerid, response, listitem, inputtext[])
{
    if(response)
    {
        ...
        SendInfoMessage(playerid, "VocÍ vendeu seu veÌculo!");
    }
    return 1;
}
...
OpenCarTrunk(playerid)
{
    ...
    if(!IsPlayerInRangeOfPoint(playerid, 5.0, x, y, z)) return SendErrorMessage(playerid, "Vocù nùo estù prùximo de nenhum veùculo!");
    if(vInfo[VehicleGetID(vehicle_id)][vLocked]) return SendErrorMessage(playerid, "Este veùculo estù trancado!");
    if(IsPlayerInAnyVehicle(playerid)) return SendErrorMessage(playerid, "Vocù nùo pode abrir um porta-malas dentro do veùculo.");
    if(GetVehicleTrunkSlots(GetVehicleModel(vehicle_id)) == -1) return SendErrorMessage(playerid, "Este veÌculo n„o tem porta-malas.");
