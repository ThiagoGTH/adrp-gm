...
bool:StartFly(playerid) {
	...
	ApplyAnimation(playerid, "PARACHUTE", "PARA_steerR", 6.1, false, false, false, false, 0);
	Fly(playerid);
	return true;
}
...
public Fly(playerid) {
	...
	if(v_x == 0 && v_y == 0) {
		...
		if(GetPlayerAnimationIndex(playerid) != 959)
		ApplyAnimation(playerid,"PARACHUTE", "FALL_SkyDive_Accel", 6.1, false, false, false, false, 0);
	} if(OnFly[playerid])
