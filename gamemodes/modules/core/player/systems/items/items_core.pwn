ItemCreate(model, price) {
    for (new i = 0; i != MAX_DYNAMIC_ITEMS; i ++) {
        if (!diInfo[i][diExists]) {
            ...
            format(diInfo[i][diName], 64, "Inv�lido");
            ...
            mysql_format(DBConn, query, sizeof query, "INSERT INTO `items` (`item_name`, `item_desc`, `item_model`, `item_category`, `item_useful`, `item_legality`, `item_price`) VALUES ('Inv�lido', 'N/A', '%d', '0', '0', '1', '%d')", model, price);
...
LoadItems() {
    ...
    printf("[ITENS DIN�MICOS]: %d itens din�micos carregados com sucesso.", loadeditems);
    return true;
}
...
ShowPlayerInventory(playerid){
	...
	format(title, sizeof(title), "Invent�rio de %s (%d/%d)", pNome(playerid), Inventory_Quantity(playerid), GetInventorySlots(playerid));
...
Dialog:PlayerInventorySelected(playerid, response, listitem, inputtext[]) {
    if (response) {
        ...
        if (!strcmp(inputtext, "Usar", true))
			return SendServerMessage(playerid, "Indispon�vel no momento.");  
