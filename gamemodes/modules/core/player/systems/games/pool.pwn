...
hook OnDialogResponse( playerid, dialogid, response, listitem, inputtext[ ] )
{
	if ( dialogid == DIALOG_POOL_WAGER )
	{
		...
		if ( poolid == -1 ) {
			return SendErrorMessage( playerid, "Unable to identify pool table. Please enter the pool table again." );
		}
		...
		if ( response && wager_amount > 0 )
		{
			if ( wager_amount > GetMoney( playerid ) ) {
				ShowPlayerDialog( playerid, DIALOG_POOL_WAGER, DIALOG_STYLE_INPUT, "{FFFFFF}Pool Wager", "{FFFFFF}Please specify the minimum entry fee for the table:\n\nYou do not have this much money!", "Set", "No" );
			} else {
				GiveMoney( playerid, -wager_amount );
				g_poolTableData[ poolid ] [ E_WAGER ] = wager_amount;
				g_poolTableData[ poolid ] [ E_READY ] = true;
				Pool_SendTableMessage( poolid, -1, "%s(%d) has set the pool wager to %s!", ReturnPlayerName( playerid ), playerid, cash_format( wager_amount ) );
				UpdateDynamic3DTextLabelText( g_poolTableData[ poolid ] [ E_LABEL ], -1, sprintf( "" # COLOR_GREY "Pool Table\n"COL_GREEN"Press ENTER To Join %s(%d)\n"COLOR_LIGHTRED"%s Entry", ReturnPlayerName( playerid ), playerid, cash_format( wager_amount ) ) );
			}
			return 1;
		}
		else
		{
			...
			g_poolTableData[ poolid ] [ E_READY ] = true;
			Pool_SendTableMessage( poolid, -1, ""COLOR_GREY"-- "COLOR_WHITE" %s(%d) has set the pool wager to FREE!", ReturnPlayerName( playerid ), playerid );
...
hook OnPlayerUpdateEx( playerid )
{
	...
	if ( IsPlayerPlayingPool( playerid ) && poolid != -1 )
	{
		...
		{
			Pool_SendTableMessage( poolid, COLOR_GREY, "-- "COLOR_WHITE" %s(%d) has been kicked from the table [Reason: Out Of Range]", ReturnPlayerName( playerid ), playerid );
		...
		if ( g_poolTableData[ poolid ] [ E_STARTED ] )
		{
			...
			if ( HOLDING( KEY_SECONDARY_ATTACK ) && IsPlayerPlayingPool( playerid ) ) {
				if ( PRESSED( KEY_CROUCH ) ) {
					...
					Pool_SendTableMessage( poolid, COLOR_GREY, "-- "COLOR_WHITE" %s(%d) has left the table", ReturnPlayerName( playerid ), playerid );
		...
		else
		{
			if ( PRESSED( KEY_SECONDARY_ATTACK ) )
			{
				...
				if ( ! IsPlayerPlayingPool( playerid ) && ! Iter_Contains( poolplayers< poolid >, playerid ) )
				{
					...
					if ( GetMoney( playerid ) < entry_fee && g_poolTableData[ poolid ] [ E_READY ] ) {
					...
					if ( pool_player_count + 1 >= 2 )
					{
						...
						Pool_SendTableMessage( poolid, COLOR_GREY, "-- "COLOR_WHITE" %s(%d) has joined the table (2/2)", ReturnPlayerName( playerid ), playerid );
					...
					else
					{
						...
						Pool_SendTableMessage( poolid, COLOR_GREY, "-- "COLOR_WHITE" %s(%d) has joined the table (1/2)", ReturnPlayerName( playerid ), playerid );
					}
					return 1;
				}
			}
				...
				if ( g_poolBallOffsetData[ poolball_index ] [ E_BALL_TYPE ] == E_STRIPED || g_poolBallOffsetData[ poolball_index ] [ E_BALL_TYPE ] == E_SOLID )
				{
					if ( ++ g_poolTableData[ poolid ] [ E_BALLS_SCORED ] == 1 )
					{
						...
						foreach ( new playerid : poolplayers< poolid > ) {
	    					...
	    					SendClientMessageFormatted( playerid, -1, ""COLOR_GREY"-- "COLOR_WHITE" %s(%d) is now playing as %s", ReturnPlayerName( first_player ), first_player, g_poolTableData[ poolid ] [ E_PLAYER_BALL_TYPE ] [ first_player ] == E_STRIPED ? ( "Striped" ) : ( "Solid" ) );
	    					SendClientMessageFormatted( playerid, -1, ""COLOR_GREY"-- "COLOR_WHITE" %s(%d) is playing as %s", ReturnPlayerName( second_player ), second_player, g_poolTableData[ poolid ] [ E_PLAYER_BALL_TYPE ] [ second_player ] == E_STRIPED ? ( "Striped" ) : ( "Solid" ) );
	    				}
	    			}
				}
...
CMD:endgame(playerid)
{
	...
	SendClientMessage(playerid, -1, ""COL_PINK"[ADMIN]"COLOR_WHITE" You have force ended the pool game!");
	return 1;
}
