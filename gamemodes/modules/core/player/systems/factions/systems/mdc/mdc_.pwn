...
CMD:mdc(playerid, params[])
{
	...
	if(IsPlayerInAnyVehicle(playerid))
	{
		...
		if(CarData[vehicleid][carFaction] == -1) return SendErrorMessage(playerid, "Herhangi bir birlik aracù iùerisinde deùilsin.");
		if(!FactionData[CarData[vehicleid][carFaction]][FactionCopPerms]) return SendClientMessage(playerid, COLOR_ADM, "SERVER: Bu araùta MDC kullanamazsùn.");

		if(GetPlayerVehicleSeat(playerid) > 1) return SendErrorMessage(playerid, "Arka koltukta MDC kullanamazsùn.");
	...
	else if(PlayerData[playerid][pInsideEntrance] != -1)
	{
		if(PlayerData[playerid][pFaction] != EntranceData[PlayerData[playerid][pInsideEntrance]][EntranceFaction])
			return SendErrorMessage(playerid, "Bu bina senin birliùine ait deùil.");

		SelectTextDraw(playerid, 0x030103FF);
		ShowMDCPage(playerid, MDC_PAGE_MAIN);
		SetPVarInt(playerid, "OnPlayerUseMDC", 1);
	}
	else SendErrorMessage(playerid, "Bu komutu kullanmak iùin birliùine ait bir binada veya birliùine ait bir araùta olmalùsùn.");
	return 1;
}
...
public ClickDynamicPlayerTextdraw(playerid, PlayerText:playertextid)
{
	...
	if(playertextid == MDC_LookUp_Name[playerid][13]) // Tutuklama Raporu
	{
		...
		else
		{
			...
			new ask_dialog[1028];
			strcat(ask_dialog, sprintf("{FFFFFF}%s adlù kiùinin zaten tutuklama raporu %s tarafùndan yazùlmùù. Silmek ister misin?", MDC_PlayerLastSearched[playerid], ReturnSQLName(by_id)));
	...
	if(playertextid == MDC_ManageLicense[playerid][6]) // Sùrùcù Lisansù iptal etme
	{
		...
		MDC_ShowManageLicense(MDC_PlastLastSearched_SQLID[playerid], playerid);
		SendLawMessage(PlayerData[playerid][pFaction], COLOR_COP, sprintf("** HQ Duyurusu: %s %s, %s adlù kiùinin ehliyetini iptal etti. **", Player_GetFactionRank(playerid), ReturnName(playerid, 1), MDC_PlayerLastSearched[playerid]));
	}
	...
	if(playertextid == MDC_ManageLicense[playerid][7]) // Sùrùcù Lisansù uyarma
	{
				...
				if(PlayerData[pl][DriversLicenseWarning] == 2)
					{
						...
						MDC_ShowManageLicense(MDC_PlastLastSearched_SQLID[playerid], playerid);
						SendLawMessage(PlayerData[playerid][pFaction], COLOR_COP, sprintf("** HQ Duyurusu: %s %s, %s adlù kiùinin ehliyetine ùùùncù uyarù sebebiyle el koydu. **", Player_GetFactionRank(playerid), ReturnName(playerid, 1), MDC_PlayerLastSearched[playerid]));
		...
		MDC_ShowManageLicense(MDC_PlastLastSearched_SQLID[playerid], playerid);
		SendLawMessage(PlayerData[playerid][pFaction], COLOR_COP, sprintf("** HQ Duyurusu: %s %s, %s adlù kiùinin ehliyetine uyarù puanù verdi. **", Player_GetFactionRank(playerid), ReturnName(playerid, 1), MDC_PlayerLastSearched[playerid]));
	}
	...
	if(playertextid == MDC_ManageLicense[playerid][14]) // Medikal Lisans iptal etme
	{
		...
		mysql_tquery(m_Handle, query);
		MDC_ShowManageLicense(MDC_PlastLastSearched_SQLID[playerid], playerid);
		SendLawMessage(PlayerData[playerid][pFaction], COLOR_COP, sprintf("** HQ Duyurusu: %s %s, %s adlù kiùinin medikal lisansùnù iptal etti. **", Player_GetFactionRank(playerid), ReturnName(playerid, 1), MDC_PlayerLastSearched[playerid]));
	}
	...
	if(playertextid == MDC_ManageLicense[playerid][30])
	{
		...
		mysql_tquery(m_Handle, query);
		MDC_ShowManageLicense(MDC_PlastLastSearched_SQLID[playerid], playerid);
		SendLawMessage(PlayerData[playerid][pFaction], COLOR_COP, sprintf("** HQ Duyurusu: %s %s, %s adlù kiùinin medikal lisansù verdi. **", Player_GetFactionRank(playerid), ReturnName(playerid, 1), MDC_PlayerLastSearched[playerid]));
	}
	...
	if(playertextid == MDC_ManageLicense[playerid][21]) // Silah lisansù verme
	{
		...
		MDC_ShowManageLicense(MDC_PlastLastSearched_SQLID[playerid], playerid);
		SendLawMessage(PlayerData[playerid][pFaction], COLOR_COP, sprintf("** HQ Duyurusu: %s %s, %s adlù kiùiye silah lisansù verdi. **", Player_GetFactionRank(playerid), ReturnName(playerid, 1), MDC_PlayerLastSearched[playerid]));
	}
	...
	if(playertextid == MDC_ManageLicense[playerid][20]) // Silah lisansù iptal
	{
		...
		mysql_tquery(m_Handle, query);
		MDC_ShowManageLicense(MDC_PlastLastSearched_SQLID[playerid], playerid);
		SendLawMessage(PlayerData[playerid][pFaction], COLOR_COP, sprintf("** HQ Duyurusu: %s %s, %s adlù kiùinin silah lisansùnù iptal etti. **", Player_GetFactionRank(playerid), ReturnName(playerid, 1), MDC_PlayerLastSearched[playerid]));
	}
...
stock MDC_AddCharge(playerid, charge)
{
	...
	SendLawMessage(PlayerData[playerid][pFaction], COLOR_COP, sprintf("** HQ Duyurusu: %s %s, %s adlù kiùi ùzerinde %d dakikalùk suùlamada bulundu. **", Player_GetFactionRank(playerid), ReturnName(playerid, 1), MDC_PlayerLastSearched[playerid], CalculateChargeTime(playerid, charge)));
	RefreshChargeButton(playerid);
	return 1;
}
...
Server:SQL_RosterList(playerid, page) // 38 geri, 39 ileri
{
	...
	if(countdown == 0)
	{
		PlayerTextDrawSetString(playerid, MDC_Roster[playerid][0], FixWord256("Liste boù."));
		PlayerTextDrawShow(playerid, MDC_Roster[playerid][strtext]);
	}
	return 1;
}
...
HandleEmergency(playerid, PlayerText:tid)
{
	...
	new handle_text[64];
	format(handle_text, sizeof(handle_text), "ùlgilenildi - %s", ReturnName(playerid));
...
Dialog:MDCCallRespond(playerid, response, listitem, inputtext[])
{
	...
	if(!listitem)
	{
		...
		ShowEmergencyCalls(playerid, GetPVarInt(playerid, "emergencylist_idx"));
		SetPVarInt(playerid, "lastEmergencyID", 0);
		SendClientMessageEx(playerid, COLOR_COP, "%d numaralù ùaùrùyù ùstlendiniz.", e_id);
	...
	else
	{
		...
		mysql_tquery(m_Handle, query);
		SetPVarInt(playerid, "lastEmergencyID", 0);
		SendClientMessageEx(playerid, COLOR_COP, "%d numaralù ùaùrùyù sildiniz.", e_id);
...
stock FixChargeDescription(string[1028])
{
	...
	ReplaceText(string, "ù", "g");
	ReplaceText(string, "ù", "G");
	ReplaceText(string, "ù", "u");
	ReplaceText(string, "ù", "U");
	ReplaceText(string, "ù", "s");
	ReplaceText(string, "ù", "S");
	ReplaceText(string, "ù", "c");
	ReplaceText(string, "ù", "C");
	ReplaceText(string, "ù", "o");
	ReplaceText(string, "ù", "O");
	ReplaceText(string, "ù", "i");
	ReplaceText(string, "ù", "I");
  return string;
}
