...
Dialog:SetRankName(playerid, response, listitem, inputtext[]) {
	...
	if (response) {
        ...
        SendServerMessage(playerid, "Você alterou o cargo da facção %s (%d) para %s (%s).", FactionData[id][factionName], id, inputtext, slot);
...
Dialog:SetPaycheck(playerid, response, listitem, inputtext[]) {
	if(response) {
        ...
        SendServerMessage(playerid, "Você alterou o salário do cargo %s da facção %s (%d) de %s para %s.", FactionGetRankName(factionid, rankid), FactionData[factionid][factionName], factionid, FormatNumber(FactionData[factionid][factionPaycheck][rankid]), FormatNumber(strval(inputtext)));

		format(logString, sizeof(logString), "%s (%s) alterou o salário do cargo %s da facção %s (%d) de %s para %s (cargo %d)", pNome(playerid), GetPlayerUserEx(playerid), FactionGetRankName(factionid, rankid), FactionData[factionid][factionName], factionid, FormatNumber(FactionData[factionid][factionPaycheck][rankid]), FormatNumber(strval(inputtext)));
		logCreate(playerid, logString, 1);
...
Dialog:FactionSkins(playerid, response, listitem, inputtext[]) {
	if(response) {
		...
		if(listitem == 0)
			...
			new string[128];
			format(string, sizeof(string), "Você deseja realmente remover a skin %d?", FactionData[factionid][factionSkins][listitem-1]);
